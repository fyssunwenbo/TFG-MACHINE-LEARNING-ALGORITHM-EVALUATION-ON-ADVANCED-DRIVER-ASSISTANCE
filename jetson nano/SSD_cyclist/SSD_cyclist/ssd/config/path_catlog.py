...
class DatasetCatalog:
    ...
    def get(name):
        if "voc" in name:
            ...
            customer_root = DatasetCatalog.DATA_DIR
            if 'CUSTOMER_ROOT' in os.environ:
                customer_root = os.environ['CUSTOMER_ROOT']

            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join(customer_root, attrs["data_dir"]),
                split=attrs["split"],
            )
            return dict(factory="CustomerDataSet", args=args)
