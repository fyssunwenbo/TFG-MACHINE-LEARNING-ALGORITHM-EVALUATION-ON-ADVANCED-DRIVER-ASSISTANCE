...
class Evaluator(object):
    def __init__(self, model=None, showatt=False,device='cuda'):
    ...
    def get_bbox(self, img, multi_test=False, flip_test=False, mode=None):
        ...
        boxes = torch.from_numpy(bboxes[:,:4]).reshape(-1,4)
        scores = torch.from_numpy(bboxes[:,4]).reshape(-1)
        labels = torch.from_numpy(bboxes[:,5]).reshape(-1)
        ...
        boxes, scores, labels = boxes[keep], scores[keep].unsqueeze(-1), labels[keep].unsqueeze(-1)
        bboxes = torch.cat([boxes,scores,labels],1).cpu().numpy()
        # bboxes = nms(bboxes, self.conf_thresh, self.nms_thresh) #原始NMS
